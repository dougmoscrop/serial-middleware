...
describe('serial-middleware', function () {
  ...
  it('supports functions of arity 4', function(done) {
    var param = serial(function(req, res, next, id) {
      assert.equal(id, 'id-param');

      counter++;
      next(null, 'otherValue');
    }, function(req, res, next, otherValue) {
      assert.equal(otherValue, 'otherValue');
      counter++;
      next();
    });

    assert.equal(param.length, 4);
